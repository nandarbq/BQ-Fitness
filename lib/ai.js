...
async function geminiGenerate(systemText, userText) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    try {
      ...
      clearTimeout(timer);
      lastErr.code = 'net'; lastErr.msg = e.message;
      continue;
    }
    clearTimeout(timer);
    const bodyText = await res.text().catch(() => '');
    if (!res.ok) {
      lastErr.code = String(res.status); lastErr.msg = bodyText.slice(0, 200);
      // Error permanen (key salah, model tidak ada, dsb) tidak perlu dicoba lagi.
      if (res.status === 401 || res.status === 403 || res.status === 400 || res.status === 404) {
        throw new Error(`Gemini ${res.status}: ${lastErr.msg}`);
      }
      continue;
    }
    let data;
    try { data = JSON.parse(bodyText); } catch { lastErr.code = 'parse'; lastErr.msg = 'Respons bukan JSON.'; continue; }
    const text = (data.candidates && data.candidates[0] && data.candidates[0].content &&
      data.candidates[0].content.parts && data.candidates[0].content.parts[0].text) || '';
    if (!text) { lastErr.code = 'empty'; lastErr.msg = 'Respons AI kosong.'; continue; }
    try { return parseJsonSafe(text); } catch (e) { lastErr.code = 'json'; lastErr.msg = e.message; }
  }
  throw new Error(`Gemini ${lastErr.code || 'err'}: ${lastErr.msg || 'tidak menjawab.'}`);
}
