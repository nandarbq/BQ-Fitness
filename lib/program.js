...
/* Umur efektif: selalu diturunkan dari tanggal lahir bila ada (agar tak perlu diisi ulang). */
function effectiveAge(row) {
  if (!row) return null;
  const byBirth = calcAge(row.birthdate);
  return byBirth != null ? byBirth : (row.age || null);
}
